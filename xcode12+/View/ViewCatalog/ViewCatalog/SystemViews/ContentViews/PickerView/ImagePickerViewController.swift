...
class ImagePickerViewController: UIViewController {
    ...
    @IBAction func shuffle(_ sender: Any?) {
        let firstIndex = Int.random(in: 0...(images.count)*3-1)
        let secondIndex = Int.random(in: 0...(images.count)*3-1)
        let thirdIndex = Int.random(in: 0...(images.count)*3-1)
...
extension ImagePickerViewController : UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return images.count * 3 // 컴포넌트의 로우수
    }
...
extension ImagePickerViewController : UIPickerViewDelegate {
    func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        // 여기서 row는 datasource에서 지정한 컴포넌트당 로우수입니다 ^^
